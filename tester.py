...
class Tester(object):
    ...
    def test(self,i_want_the_predict_result=False,predict_result_saver:list=None):
        r"""开始进行验证，并返回验证结果。
        i_want_the_predict_result  想不想要预测的index逻辑值
        predict_result_saver，默认用来接收数据的list，否则return 元组的第二个对象
        ...
        try:
            with torch.no_grad():
                ...
                with inner_tqdm(total=len(data_iterator), leave=False, dynamic_ncols=True) as pbar:
                    ...
                    if i_want_the_predict_result and predict_result_saver ==None:
                        result_saver =[]
                    for batch_x, batch_y in data_iterator:
                        _move_dict_value_to_device(batch_x, batch_y, device=self._model_device)
                        pred_dict = self._data_forward(self._predict_func, batch_x)
                        if i_want_the_predict_result:
                            if predict_result_saver !=None:
                                predict_result_saver.append(pred_dict)
                            else:
                                result_saver.append(pred_dict)
        ...
        if i_want_the_predict_result and predict_result_saver ==None:
            return  eval_result,result_saver
        return eval_results
